      ...
      try {
        ...
        if (result.success) {
          ...
          const customNames = new Set(
            custom.map((c) => c.name.toLowerCase().trim())
          );
          const filteredStandard = standard.filter(
            (s) => !customNames.has(s.name.toLowerCase().trim())
          );
      ...
      if (isCustom) {
        ...
        if (result.success && companyResult.success) {
          setGapData({
            ...result.data,
            about:
              companyResult.data.aboutHiring ||
              "Top industry leader in product engineering.",
            process: companyResult.data.hiringProcess || [
              "Aptitude",
              "Technical",
              "HR",
            ],
              ...
              {companies.map((company, i) => (
                ...
                >
                  <div className="mb-6">
                    <div className="flex justify-between items-start mb-6">
                      ...
                      <div className="flex flex-col items-end gap-1.5">
                        {company.isCustom ? (
                          <span className="px-2.5 py-1 bg-[#EC4899] text-white rounded-lg text-[7px] font-black uppercase tracking-[0.2em] shadow-lg shadow-[#EC4899]/30">
                            Custom
                          </span>
                        ) : (
                          <span className="px-2 py-0.5 border border-gray-100 text-gray-300 rounded-md text-[7px] font-black uppercase tracking-widest">
                            Global
                          </span>
                        )}
                        <span
                          className={`px-2 py-0.5 rounded-md text-[8px] font-black uppercase tracking-widest ${company.fit > 0 ? "bg-[#E8FAF5] text-[#009D77]" : "bg-orange-50 text-orange-600"}`}
                        >
                          {company.fit}% Match
                        </span>
                      </div>
                    </div>
                    <h3 className="text-base font-black text-[#011813] uppercase tracking-tight mb-1 transition-colors group-hover:text-[#009D77]">
                      {company.name}
                    </h3>
                    <div className="flex items-center gap-2 text-[9px] font-black text-gray-300 uppercase tracking-widest leading-none">
                      <MapPin className="w-3 h-3 text-[#009D77]/40" />{" "}
                      {company.location}
                    </div>
                  </div>
                  <div className="pt-5 border-t border-gray-50 flex justify-between items-center text-[8px] font-black uppercase tracking-widest">
                    <span className="text-gray-300 group-hover:text-[#009D77] transition-colors">
                      {company.isCustom ? "Private Pipeline" : "Sync Active"}
                    </span>
                    {company.isCustom ? (
                      <button
      ...
      {selectedCompanyId && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <motion.div
            ...
            className="relative bg-white w-full max-w-3xl max-h-[90vh] overflow-y-auto rounded-[2rem] shadow-2xl p-6 md:p-8 flex flex-col"
          >
            <button
              onClick={() => setSelectedCompanyId(null)}
            ...
            {fetchingGap ? (
              <div className="py-20 flex flex-col items-center">
                ...
                <p className="font-black text-[9px] uppercase tracking-widest text-gray-400">
                  Benchmarking Stack Requirements...
                </p>
              </div>
            ) : gapData ? (
              <div className="space-y-6">
                ...
                <div className="flex items-end justify-between border-b border-gray-50 pb-6">
                  <div>
                    <div className="flex items-center gap-3 mb-1">
                      <h2 className="text-2xl font-black text-[#011813] uppercase tracking-tight">
                        {gapData.company}
                      </h2>
                      <span className="px-2 py-0.5 bg-[#EC4899] text-white rounded text-[8px] font-black uppercase tracking-widest">
                        Enterprise Pro
                      </span>
                    </div>
                    <p className="text-[#009D77] font-black uppercase tracking-[0.2em] text-[9px]">
                      Industry Segment: {gapData.sector}
                    </p>
                  </div>
                  <div className="text-right">
                    <p className="text-[9px] font-black text-gray-400 uppercase tracking-widest mb-1.5 leading-none">
                      Your Compatibility
                    </p>
                    <p className="text-4xl font-extrabold text-[#009D77] leading-none">
                      {gapData.currentMatchPercent}%
                    </p>
                  </div>
                </div>
                ...
                <div className="bg-[#fbfcfa] p-5 rounded-2xl border border-gray-50">
                  <h3 className="text-[10px] font-black text-[#475467] uppercase tracking-widest mb-2">
                    Technical Culture & Hiring
                  </h3>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  ...
                  <div className="md:col-span-2 space-y-4">
                    <div className="flex items-center justify-between">
                      <h3 className="text-[10px] font-black text-[#475467] uppercase tracking-widest">
                        Skill Alignment Benchmark
                      </h3>
                      <span className="text-[8px] font-black text-[#009D77] uppercase bg-[#E8FAF5] px-2 py-0.5 rounded tracking-widest">
                        Gaps Detected
                      </span>
                    </div>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      {gapData.topGaps.map((gap, idx) => (
                        <div
                          key={idx}
                          className="bg-white border border-gray-100 p-4 rounded-xl relative group hover:border-[#EC4899]/30 transition-all"
                        >
                          <div className="flex justify-between items-start mb-2">
                            <h4 className="text-[10px] font-black text-[#011813] uppercase tracking-tight leading-none">
                              {gap.skill}
                            </h4>
                            <span className="text-[7px] font-black text-rose-500 uppercase tracking-[0.2em]">
                              Gap: {gap.depthGap} Lv
                            </span>
                          </div>
                          <p className="text-[8px] font-medium text-gray-400 mb-3 leading-tight line-clamp-2">
                            {gap.actionToFix}
                          </p>
                          <div className="flex items-center gap-2">
                            <div className="flex-1 h-1 bg-gray-50 rounded-full overflow-hidden">
                              <div
                                ...
                                style={{
                                  width: `${(gap.studentDepth / gap.minimumDepth) * 100}%`,
                                }}
                              />
                            </div>
                            <span className="text-[7px] font-black text-gray-300">
                              {gap.studentDepth}/{gap.minimumDepth}
                            </span>
                          </div>
                        </div>
                      ))}
                    ...
                    {/* Hiring Process */}
                    <div className="mt-4">
                      <h3 className="text-[10px] font-black text-[#475467] uppercase tracking-widest mb-3">
                        Interview Pipeline
                      </h3>
                      <div className="flex flex-wrap gap-2">
                        {gapData.process.map((step, idx) => (
                          <div
                            key={idx}
                            className="bg-gray-50 border border-gray-100 px-3 py-1.5 rounded-lg flex items-center gap-2"
                          >
                            <span className="w-4 h-4 bg-[#011813] text-white rounded-full flex items-center justify-center text-[8px] font-black">
                              {idx + 1}
                            </span>
                            <span className="text-[9px] font-black text-[#011813] uppercase tracking-widest">
                              {step}
                            </span>
                          </div>
                        ))}
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="bg-[#011813] text-white p-6 rounded-2xl shadow-xl relative overflow-hidden h-fit flex flex-col justify-between min-h-[160px]">
                      ...
                      <p className="text-[10px] font-bold leading-relaxed mb-6 opacity-80 uppercase tracking-tight text-white/90 italic">
                        "{gapData.verdict}"
                      </p>
                      <div className="border-t border-white/10 pt-4">
                        <div className="flex justify-between items-end text-[7px] font-black text-white/40 uppercase tracking-[0.2em] mb-1">
                          ...
                          <span className="text-white text-base font-black leading-none">
                            {gapData.projectedMatchIfAllFixed}%
                          </span>
                        </div>
                      </div>
                    </div>
                    <button
                      onClick={() =>
                        (window.location.href = `/roadmap?generateFor=${gapData.companyId}`)
                      }
        ...
        {showAddModal && (
          ...
          >
            ...
            <motion.div
              ...
              className="relative bg-white w-full max-w-xl rounded-[2rem] shadow-2xl p-8 overflow-hidden"
            >
              <div className="absolute top-0 right-0 w-32 h-32 bg-[#009D77]/5 rounded-full -mr-16 -mt-16 blur-2xl" />
              <h2 className="text-xl font-black text-[#011813] uppercase tracking-tighter mb-1">
                Add Custom Benchmark
              </h2>
              <p className="text-[#3b4b45]/60 text-[10px] font-bold uppercase tracking-widest mb-6">
                target specific industry pipelines
              </p>
              ...
              >
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1.5">
                    ...
                    <input
                      ...
                      className="w-full bg-gray-50 border border-gray-100 rounded-xl px-4 py-3 text-sm font-bold focus:border-[#009D77] outline-none transition-all"
                    />
                  </div>
                  <div className="space-y-1.5">
                    <label className="text-[9px] font-black text-gray-400 uppercase tracking-widest pl-1">
                ...
                <div className="p-4 bg-[#fbfcfa] rounded-2xl border border-dashed border-gray-200">
                  <p className="text-[8px] font-black text-gray-400 uppercase tracking-widest text-center">
                    AI will automatically benchmark against role standards
                  </p>
                </div>
                <div className="flex gap-3 pt-4">
                  <button
                    type="button"
                    onClick={() => setShowAddModal(false)}
                    className="flex-1 py-4 bg-gray-50 text-gray-400 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-gray-100 transition-all"
                  >
                    Cancel
                  </button>
                  <button
                    type="submit"
                    className="flex-2 px-12 py-4 bg-[#009D77] text-white rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-[#011813] transition-all shadow-lg shadow-[#009D77]/20"
                  >
                    Create Target
                  </button>
                </div>
