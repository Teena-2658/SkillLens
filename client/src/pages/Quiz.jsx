      ...
      <div className="h-screen bg-[#F8F9FA] flex flex-col items-center justify-center px-4 sm:px-6 relative overflow-hidden">
        ...
        >
           ...
           <div className="absolute top-0 left-0 h-1.5 bg-[#F8F9FA] w-full rounded-t-2xl overflow-hidden">
             <motion.div 
               initial={{ width: 0 }}
               animate={{ width: `${((currentQuestionIdx) / activeQuiz.length) * 100}%` }}
           ...
           <div className="flex justify-between items-center mb-6 mt-2">
             <span className="text-[#009D77] font-extrabold uppercase tracking-wider text-[10px] flex items-center gap-1.5">
               <Target className="w-3.5 h-3.5" /> Q {currentQuestionIdx + 1} OF {activeQuiz.length}
             </span>
           ...
           <div className="space-y-3">
             {q.options.map((opt, idx) => (
               ...
               >
                 <span className={`inline-flex items-center justify-center w-6 h-6 rounded bg-white shadow-sm border mr-4 text-[10px] font-bold transition-colors ${answers[currentQuestionIdx]?.selectedIndex === idx ? 'border-[#009D77] text-[#009D77]' : 'border-[#E7E7E8] text-[#98A2B3] group-hover:text-[#475467]'}`}>
      ...
      <main className="w-full max-w-6xl mx-auto flex flex-col h-[88vh] max-h-[800px] gap-4 relative z-10">
        ...
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="flex justify-between items-center bg-white border border-[#E7E7E8] rounded-2xl p-4 shadow-sm shrink-0">
          ...
          <div className="hidden md:flex items-center gap-3 bg-[#F8F9FA] border border-[#E7E7E8] p-2 rounded-xl">
             ...
             <div>
               <p className="text-[9px] font-bold text-[#98A2B3] uppercase tracking-wider mb-0.5">Ranking Rank</p>
               <p className="text-[11px] font-extrabold text-[#011813]">Top 15% User</p>
             </div>
          </div>
        </motion.div>
        ...
        <div className="flex-1 bg-white border border-[#E7E7E8] rounded-2xl shadow-sm p-4 overflow-hidden flex flex-col min-h-0">
          {loading ? (
             <div className="flex flex-col items-center justify-center flex-1">
               ...
               <p className="text-sm font-bold text-[#475467]">Fetching assessments...</p>
             </div>
          ) : (
                  ...
                  {quizzes.map((quiz, index) => (
                    ...
                    >
                      <div>
                        ...
                        <div className="flex justify-between items-start mb-4">
                           ...
                           {quiz.completed ? (
                              <span className="bg-[#E8FAF5] text-[#009D77] px-2 py-1 rounded text-[9px] font-bold flex items-center gap-1 shadow-sm">
                                <CheckCircle2 className="w-2.5 h-2.5" /> {quiz.score}
                              </span>
                           ) : (
                        ...
                        {/* Meta Tags */}
                        <div className="flex items-center gap-3 text-[10px] font-semibold text-[#98A2B3] mb-5">
                           <span className="flex items-center gap-1"><Clock className="w-3 h-3 group-hover:text-[#009D77] transition-colors" /> {quiz.time}</span>
                           <span className="flex items-center gap-1"><Target className="w-3 h-3 group-hover:text-[#009D77] transition-colors" /> {quiz.questions} Q</span>
                        </div>
                      </div>
